...
$item->StartPrice = new Types\AmountType(['value' => ($add_product_array['list_amount']/100)]);
//echo "Ebay Page";
//echo "<pre>";
//print_r($item);
//print_r($new_image_array);
//print_r($add_product_array);
//print_r($config);
//echo "</pre>";
//exit;
/**
 ...
 */
if(isset($sale_price) && $sale_price!=$new_price)
{
    $item->ListingDetails = new Types\ListingDetailsType();
    $item->ListingDetails->BestOfferAutoAcceptPrice = new Types\AmountType(['value' => ($new_price/100)]);
    $item->ListingDetails->MinimumBestOfferPrice = new Types\AmountType(['value' => ($sale_price/100)]);
    $item->ListingDetails->BestOfferAutoAcceptPrice = new Types\AmountType(['value' => ($new_price/100)]);
    $item->ListingDetails->MinimumBestOfferPrice = new Types\AmountType(['value' => ($sale_price/100)]);
}
 ...
 */
$item->Title = substr($add_product_array['title'],0,80);
$item->Description = $add_product_array['description'];
$item->SKU = 'vivek-001';
 ...
 */
$item->Currency = $add_product_array['currency_code'];
...
$item->PictureDetails->PictureURL = $new_image_array;
/**
...
if ($response->Ack !== 'Failure') {
    printf(
            "The item was listed to the eBay Sandbox with the Item number %s\n http://cgi.sandbox.ebay.com/ws/eBayISAPI.dll?ViewItem&item=", $response->ItemID
    );
}
?>