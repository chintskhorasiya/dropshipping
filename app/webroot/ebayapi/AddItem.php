        ...
        <div class="container" style="width:50%">
            <form action="AddItem.php" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6"><input class="form-control" type="text" name="searched_keyword" readonly="readonly" value="<?php echo (isset($_POST['search_keyword']) && $_POST['search_keyword']!='')?$_POST['search_keyword']:''; ?>"></div>
                </div>
...
//            exit;
            ...
            $itemTitle = $add_product_array['title'];
            $startPrice =  ($add_product_array['list_amount']/100);
            $buyItNowPrice = 0;

//            $buyItNowPrice = ($add_product_array['list_amount']/100);
            $itemDescription = $add_product_array['description'];
            $img_name = $new_image_array;
            $listingDuration = 'Days_30';   //14,30 days
            $primaryCategory = 29792;
            ...
            //if (isset($_POST['submit']))
            {
                ...
                $listingType = $_POST['listingType'];   //Fixed price
                $primaryCategory = $_POST['primaryCategory'];   //categoryid
                $itemTitle = $_POST['itemTitle'];
                $startPrice = $_POST['startPrice'];
                  $buyItNowPrice   = $_POST['buyItNowPrice'];
                $listingDuration = $_POST['listingDuration'];   //14,30 days
                $safequery = $_POST['searched_keyword'];

                if (get_magic_quotes_gpc()) {
                    // print "stripslashes!!! <br>\n";
                    $itemDescription = stripslashes($add_product_array['description']);
                } else {
                    $itemDescription = $add_product_array['description'];
...
//                echo "<br>";
//                echo 'itemCondition '.
                //$itemCondition = $_POST['itemCondition'];   //New, Used
                $itemCondition = 1000; //1000 for new, 3000 for used
//                echo "<br>";
...
//                $requestXmlBody .= '</BestOfferDetails>';
                ...
                $requestXmlBody .= $picturexmldata; // set image data from get_product_detail.php file
                //
...
//                endfor;
                ...
                $requestXmlBody .= "<BuyItNowPrice currencyID='".$add_product_array['currency_code']."'>$buyItNowPrice</BuyItNowPrice>";

                if(isset($add_product_array['source_id']) && $add_product_array['source_id']==1)
                    $requestXmlBody .= '<Country>US</Country>';
                else
                    $requestXmlBody .= '<Country>UK</Country>';

                $requestXmlBody .= '<Currency>'.$add_product_array['currency_code'].'</Currency>';
